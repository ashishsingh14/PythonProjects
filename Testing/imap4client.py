...
def cbPickMailbox(result, proto, mboxes):
    ...
    return proto.examine(mbox
        ).addCallback(cbExamineMbox1, proto
        )
...
def cbExamineMbox(result, proto):
    ...
    return proto.fetchSpecific('1:*',
                               ...
                               headerArgs=['SUBJECT'],
        ).addCallback(cbFetch1, proto
        )

def cbExamineMbox1(result, proto):
    """
    Callback invoked when examine command completes.

    Retrieve the subject header of every message in the mailbox.
    """
    return proto.fetchFull('1:*'
        ).addCallback(cbFetch1, proto
        )
...
def cbBody(result, proto):
    """
    Display body of the message
    """
    if result:
        keys = result.keys()
        keys.sort()
